  ...
  useEffect(() => {
      ...
      if (bookId) {
        try {
          const response = await fetch(
            `http://localhost:4000/api/chapters/${bookId}`
          );
          if (!response.ok) {
    ...
    async function fetchVerses(chapterNumber) {
      if (chapterNumber) {
        try {
          const response = await fetch(
            `http://localhost:4000/api/api/verses/${chapterNumber}`
          );
          if (!response.ok) {
            throw new Error("Network response was not ok");
          }
          ...
          <ul className="grid w-full grid-cols-1 gap-10 p-10 m-5 justify-items-center xl:max-w-screen-xl lg:max-w-screen-lg md:grid-cols-3">
            {chaptersToShow.map((chapter) => (
              <li
                ...
                className="justify-center w-full mb-4 shadow-2xl card-body card bg-gray-50 outline-5"
              >
                <Link
                  href={`/ChapterInfo?chapterNumber=${chapter.chapter_number}`}
                >
                  <h2 className="p-1 text-xl font-semibold">
